...
A capstone project applying **K-Nearest Neighbors (KNN)** classification to
predict whether an e-commerce session will end in a purchase. The pipeline
covers exploratory analysis, preprocessing, stratified splitting, k-tuning,
training, and evaluation, with the classifier and supporting utilities
...
- Stratified 80/20 split — shuffle each class's indices, take the first 20%
  of each as the test set so class proportions are preserved.
...
- Compute Euclidean distances from each test point to every training point,
  take the `k` smallest, majority-vote the labels.
- Reports accuracy, precision, recall, F1, and specificity alongside the
  TP/TN/FP/FN counts.
